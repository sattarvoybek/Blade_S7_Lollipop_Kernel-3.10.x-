...
static int diagchar_close(struct inode *inode, struct file *file)
{
	...
	pr_info("diag:process exit %s, tgid =%d, logging_process_id= %d,\n", 
		current->comm, current->tgid, driver->logging_process_id);
...
static int diag_switch_logging(int requested_mode)
{
	...
	pr_info("diag:%s, task%s,tgid=%d, logging_process_id=%d, driver->logging_mode=%d\n",
		__func__, current->comm, current->tgid, driver->logging_process_id, driver->logging_mode);
