...
extern int32_t ov13850_set_otp(struct msm_sensor_ctrl_t *s_ctrl);
#endif
//end
...
int32_t back_camera_proc_file(struct msm_sensor_ctrl_t *s_ctrl)
{
    ...
    {
        sprintf(g_back_camera_status,"%s","BACK Camera ID: ov13850 13M");
    }
    //yuxin add for P839F35 back camera info 2015.07.23
    if(!strcmp("s5k3l2",s_ctrl->sensordata->sensor_name))
    {
        sprintf(g_back_camera_status,"%s","BACK Camera ID: s5k3l2 13M");
    }
...
int32_t front_camera_proc_file(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if(!strcmp("t4k35",s_ctrl->sensordata->sensor_name))
    {
        sprintf(g_front_camera_status,"%s","FRONT Camera ID: t4k35 8M");
    }

    //yuxin add for P839F35 front camera info 2015.07.23
    if(!strcmp("s5k3m2",s_ctrl->sensordata->sensor_name))
    {
        sprintf(g_front_camera_status,"%s","FRONT Camera ID: s5k3m2 13M");
    }
