					...
					struct fpc1020_platform_data *pdata)
{
	...
	/* required properties */
	int irq_prop = of_get_named_gpio(node, "fpc,gpio_irq", 0);
	int rst_prop = of_get_named_gpio(node, "fpc,gpio_reset", 0);
	int cs_prop  = of_get_named_gpio(node, "fpc,gpio_cs", 0);
	...
	pdata->cs_gpio    = (cs_prop >= 0) ? cs_prop : -EINVAL;
