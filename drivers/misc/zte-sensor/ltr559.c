...
struct ltr559_data {
	...
	struct input_polled_dev *als_input_dev; 
	...
	/* PS LED */
	int ps_led;
	int ps_led_pulse_count;
	int ps_led_measure_rate;
...
int ps_led = 0x7F; // ps_led_freq=60kHz; led_duty_cycle=100%; led current=100mA
int ps_led_pulse_count = 0x8; // 8
int ps_led_measure_rate = 0xA; // 10ms
int als_meas_rate = 0x3; // als_integration_time=100ms; als_meas_rate=500 ms
int als_contr = 0xD; // als gain = 8x; sw reset=0; als mode=active mode
...
static uint16_t find_avg (uint16_t *ps_val)
{
	uint8_t i = 0;
	uint16_t sum = 0, avg = 0;

	for(; i < PS_CAL_DATA_CNT; i++)
		sum += ps_val[i];
	
	avg = sum / PS_CAL_DATA_CNT;
...
static void report_ps_input_event(struct ltr559_data *ltr559)
{
	...
	printk("ljs %s low_threshold=%d high_threshold=%d ps_value=%d\n", 
			__func__, sensor_info->ps_lowthresh, sensor_info->ps_highthresh, adc_value);
...
static int8_t ps_enable(void)
{
	static int8_t rc = 0;
	uint8_t buffer[1]; // for dummy read

	rc = set_ps_range(sensor_info->ps_lowthresh, sensor_info->ps_highthresh, LO_N_HI_LIMIT, sensor_info);
	if (rc < 0) {
		dev_err(&sensor_info->i2c_client->dev, "%s : PS Thresholds Write Fail...\n", __func__);
		return rc;
	}

	rc = ps_led_setup(sensor_info->ps_led, sensor_info);
	if (rc < 0) {
		dev_err(&sensor_info->i2c_client->dev, "%s: PS LED Setup Fail...\n", __func__);
		return rc;
	}

	rc = ps_ledPulseCount_setup(sensor_info->ps_led_pulse_count, sensor_info);
	if (rc < 0) {
		dev_err(&sensor_info->i2c_client->dev, "%s: PS LED pulse count setup Fail...\n", __func__);
	}

	rc = ps_meas_rate_setup(sensor_info->ps_led_measure_rate, sensor_info);
	if (rc < 0) {
		dev_err(&sensor_info->i2c_client->dev, "%s: PS MeasRate Setup Fail...\n", __func__);
		return rc;
	}

	rc = ps_mode_setup(SET_BIT, sensor_info);
	if (rc < 0) {
...
{
	uint16_t adc_value;
	uint16_t	ps_init_kept_data[PS_CAL_DATA_CNT];
	uint16_t	ps_avg;
	uint32_t	ftn_init;
	uint32_t	ftn_final;
	uint32_t	ntf_final;
	...
	for(; ps_kept_data_counter<PS_CAL_DATA_CNT; ps_kept_data_counter++)
	{
		mdelay(150);
	...
	ps_avg = find_avg(ps_init_kept_data);
	ftn_init = ps_avg * 15;

	if((ftn_init - ps_avg*10) < 1000)
		ftn_final = ps_avg*10 + 1000;
	else
		if((ftn_init - ps_avg*10) > 1500)
			ftn_final = ps_avg*10 + 1500;
		else
			ftn_final = ftn_init;

	ntf_final = ftn_final - ps_avg*10;
	ntf_final *= 4;
	ntf_final /= 100;
	ntf_final += ps_avg;
	ftn_final /= 10;
	...
	if(ftn_final > 1500)
	{
		printk("ljs %s calibration result is too big, use default threshold instead\n", __func__);
		goto err;
	}
		
	sensor_info->ps_highthresh = ftn_final; 
	sensor_info->ps_lowthresh = ntf_final;
...
static ssize_t ps_led_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", sensor_info->ps_led);
}

static ssize_t ps_led_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long val;

	if (strict_strtoul(buf, 10, &val))
		return -EINVAL;
	dev_info(&sensor_info->i2c_client->dev, "PS led set %lx", val);

	if((val >= 0) && (val <= 0xFF))
		sensor_info->ps_led = val;

	return count;
}
static DEVICE_ATTR(prox_led, S_IRUGO | S_IWUSR, ps_led_show, ps_led_store);
...
static ssize_t ps_enable_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (strict_strtoul(buf, 10, &enable))
		return -EINVAL;
	dev_info(&sensor_info->i2c_client->dev, "~~~~~PS set poll enable %ld~~~~", enable);
	...
	if(enable)
	{
		...
		// report initial state FAR
		input_report_abs(sensor_info->ps_input_dev, ABS_DISTANCE, FAR_VAL);
		input_sync(sensor_info->ps_input_dev);
	}
	else
	{
...
static int8_t als_enable(void)
{
	int8_t rc = 0;
	uint8_t buffer[1]; // for dummy read

	rc = als_meas_rate_reg_setup(sensor_info->als_meas_rate, sensor_info);
	if (rc < 0) {
		dev_err(&sensor_info->i2c_client->dev, "%s: ALS_Meas_Rate register Setup Fail...\n", __func__);
		return rc;
	}

	rc = als_contr_setup(sensor_info->als_contr, sensor_info);
	if (rc < 0) {
...
static int8_t als_disable(void)
{
	int8_t rc = 0;
...
static ssize_t als_enable_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (strict_strtoul(buf, 10, &enable))
		return -EINVAL;
	dev_info(&sensor_info->i2c_client->dev, "~~~~~ALS set poll enable %ld~~~~", enable);
...
static int als_polled_input_setup(struct ltr559_data *ltr559)
{
	int ret;
...
static int ps_input_setup(struct ltr559_data *ltr559)
{
	int ret;
...
static int8_t _check_part_id(struct ltr559_data *ltr559)
{
	int8_t ret;
	uint8_t buffer[2];
...
static int ltr559_setup(struct ltr559_data *ltr559)
{
	...
	ret =  request_irq( ltr559->gpio_int, ltr559_irq_handler, IRQ_TYPE_EDGE_FALLING, DEVICE_NAME, ltr559);
	if (ret) {
	...
	/* Enable only PS interrupts on the device and clear only when status is read */
...
/* lijiangshuo add start 20141226 */
	if (client->dev.of_node)
	{
		...
		{
			ltr559->int_gpio = temp_val;
			printk("ljs %s ltr559 irq get gpio %d\n", __func__, ltr559->int_gpio);
			gpio_request(ltr559->int_gpio, "ltr559_irq");
			gpio_direction_input(ltr559->int_gpio);
			client->irq = gpio_to_irq(ltr559->int_gpio);
		}
		else
		{
...
/* lijiangshuo add end 20141226*/
	...
	ltr559->ps_led = ps_led;
	ltr559->ps_led_pulse_count = ps_led_pulse_count;
	ltr559->ps_led_measure_rate = ps_led_measure_rate;
	ltr559->als_meas_rate = als_meas_rate;
	ltr559->als_contr = als_contr;
...
err_out3:
	sensor_power_onoff(0); // lijiangshuo add 20150213
err_out2:
	gpio_free(ltr559->int_gpio);
err_out1:
	kfree(ltr559);
...
static void __exit ltr559_exit(void)
{
	...
	gpio_free(sensor_info->int_gpio);
	sensor_power_onoff(0); // lijiangshuo add 20150213
