...
#if 0
#define SENSITIVITY_2G		3906	/**	ug/LSB	*/
...
static struct proc_dir_entry *n2dm_info_proc_file;

static int n2dm_acc_i2c_read(struct n2dm_acc_status *stat, u8 *buf, int len)
{
	int ret;
	u8 reg = buf[0];
	u8 cmd = reg;
	...
	if (stat->use_smbus) {
		if (len == 1) {
			...
			buf[0] = ret & 0xff;
#ifdef DEBUG
			dev_warn(&stat->client->dev, "i2c_smbus_read_byte_data: ret=0x%02x, len:%d ,command=0x%02x, buf[0]=0x%02x\n", ret, len, cmd , buf[0]);
#endif
		} else if (len > 1) {
#ifdef DEBUG
			unsigned int ii;
			...
			ret = i2c_smbus_read_i2c_block_data(stat->client,
								cmd, len, buf);
#ifdef DEBUG
			dev_warn(&stat->client->dev,"i2c_smbus_read_i2c_block_data: ret:%d len:%d, command=0x%02x, ", ret, len, cmd);
			for (ii = 0; ii < len; ii++)
				printk(KERN_DEBUG "buf[%d]=0x%02x,", ii, buf[ii]);
			printk("\n");
#endif
		} else
			ret = -1;
...
static int n2dm_acc_i2c_write(struct n2dm_acc_status *stat, u8 *buf, int len)
{
	int ret;
	u8 reg, value;
	...
	if (stat->use_smbus) {
		if (len == 1) {
			ret = i2c_smbus_write_byte_data(stat->client,
								reg, value);
#ifdef DEBUG
			dev_warn(&stat->client->dev, "i2c_smbus_write_byte_data: ret=%d, len:%d, command=0x%02x, value=0x%02x\n",
						ret, len, reg , value);
#endif
			return ret;
		} else if (len > 1) {
			...
			ret = i2c_smbus_write_i2c_block_data(stat->client,
							reg, len, buf + 1);
#ifdef DEBUG
			dev_warn(&stat->client->dev, "i2c_smbus_write_i2c_block_data: ret=%d, len:%d, command=0x%02x, ",
							ret, len, reg);
			for (ii = 0; ii < (len + 1); ii++)
				printk(KERN_DEBUG "value[%d]=0x%02x,", ii, buf[ii]);
			printk("\n");
#endif
			return ret;
		}
...
//	pr_info("%s: hw init start\n", N2DM_ACC_DEV_NAME);
	...
	chip_id = buf[0] + 1; // differ from lis3dh
...
static int n2dm_acc_update_fs_range(struct n2dm_acc_status *stat, u8 new_fs_range)
{
	int err = -1;
	u32 sensitivity;
	u8 buf[2];
...
static int n2dm_acc_update_odr(struct n2dm_acc_status *stat, int poll_interval_ms)
{
	int err = -1;
	int i;
	u8 config[2];
	...
	for (i = ARRAY_SIZE(n2dm_acc_odr_table) - 1; i >= 0; i--) {
		if ((n2dm_acc_odr_table[i].cutoff_ms <= poll_interval_ms) || (i == 0))
			break;
	}
...
static int n2dm_acc_get_acceleration_data(struct n2dm_acc_status *stat, int *xyz)
{
	int err = -1;
...
static void n2dm_acc_report_values(struct n2dm_acc_status *stat, int *xyz)
{
...
static void n2dm_acc_input_work_func(struct work_struct *work)
{
	...
	printk("%s in\n", __func__);
	stat = container_of((struct delayed_work *)work, struct n2dm_acc_status, input_work);
...
static int n2dm_acc_validate_pdata(struct n2dm_acc_status *stat)
{
	...
	stat->pdata->min_interval =
		max((unsigned int)N2DM_ACC_MIN_POLL_PERIOD_MS, stat->pdata->min_interval);
	...
	if (stat->pdata->axis_map_x > 2 || stat->pdata->axis_map_y > 2 || stat->pdata->axis_map_z > 2) {
		dev_err(&stat->client->dev, "invalid axis_map value x:%u y:%u z%u\n", 
			stat->pdata->axis_map_x, stat->pdata->axis_map_y, stat->pdata->axis_map_z);
		return -EINVAL;
	}
	...
	if (stat->pdata->negate_x > 1 || stat->pdata->negate_y > 1 || stat->pdata->negate_z > 1) {
		dev_err(&stat->client->dev, "invalid negate value x:%u y:%u z:%u\n", 
			stat->pdata->negate_x, stat->pdata->negate_y, stat->pdata->negate_z);
		return -EINVAL;
	}
...
static int n2dm_acc_input_init(struct n2dm_acc_status *stat)
{
	...
	stat->input_dev->name = "zte_acc"; //N2DM_ACC_DEV_NAME;
...
static int n2dm_parse_dt(struct device *dev, struct n2dm_acc_platform_data *pdata)
{
	struct device_node *np = dev->of_node;
	u32 temp_val;
	int rc;

	rc = of_property_read_u32(np, "min-interval", &temp_val);
	if (rc && (rc != -EINVAL)) {
	...
	rc = of_property_read_u32(np, "poll-interval", &temp_val);
	if (rc && (rc != -EINVAL)) {
	...
	rc = of_property_read_u32(np, "axis-map-x", &temp_val);
	if (rc && (rc != -EINVAL)) {
	...
	rc = of_property_read_u32(np, "axis-map-y", &temp_val);
	if (rc && (rc != -EINVAL)) {
	...
	rc = of_property_read_u32(np, "axis-map-z", &temp_val);
	if (rc && (rc != -EINVAL)) {
	...
	rc = of_property_read_u32(np, "fs-range", &temp_val);
	if (rc && (rc != -EINVAL)) {
	...
	pdata->gpio_int1 = of_get_named_gpio_flags(dev->of_node, "gpio-int1", 0, NULL);
	pdata->gpio_int2 = of_get_named_gpio_flags(dev->of_node, "gpio-int2", 0, NULL);
	return 0;
}
		...
		const struct i2c_device_id *id)
{
	...
	if(client->dev.of_node)
	{
		...
		err = n2dm_parse_dt(&client->dev, stat->pdata);
		if (err) {
	...
	{
		stat->pdata = client->dev.platform_data;
		printk("use  platform data\n");
	}
	...
	if (stat->pdata->gpio_int1 >= 0) {
		...
		pr_info("%s: %s has set irq1 to irq: %d, mapped on gpio:%d\n",
			N2DM_ACC_DEV_NAME, __func__, stat->irq1, stat->pdata->gpio_int1);
	}
	if (stat->pdata->gpio_int2 >= 0) {
		stat->irq2 = gpio_to_irq(stat->pdata->gpio_int2);
		pr_info("%s: %s has set irq2 to irq: %d, mapped on gpio:%d\n",
			N2DM_ACC_DEV_NAME, __func__, stat->irq2, stat->pdata->gpio_int2);
	}

	memset(stat->resume_state, 0, ARRAY_SIZE(stat->resume_state));

	stat->resume_state[RES_CTRL_REG1] = (ALL_ZEROES |N2DM_ACC_ENABLE_ALL_AXES);
	...
	if (err < 0) {
		dev_err(&client->dev, "device N2DM_ACC_DEV_NAME sysfs register failed\n");
	...
	if (stat->pdata->gpio_int1 >= 0) {
		...
		stat->irq1_work_queue = create_singlethread_workqueue("n2dm_acc_wq1");
		if (!stat->irq1_work_queue) {
			err = -ENOMEM;
			dev_err(&client->dev, "cannot create work queue1: %d\n", err);
			goto err_remove_sysfs_int;
		}
		err = request_irq(stat->irq1, n2dm_acc_isr1, IRQF_TRIGGER_RISING, "n2dm_acc_irq1", stat);
		if (err < 0) {
	...
	if (stat->pdata->gpio_int2 >= 0) {
		...
		stat->irq2_work_queue = create_singlethread_workqueue("n2dm_acc_wq2");
		if (!stat->irq2_work_queue) {
			err = -ENOMEM;
			dev_err(&client->dev, "cannot create work queue2: %d\n", err);
			goto err_free_irq1;
		}
		err = request_irq(stat->irq2, n2dm_acc_isr2, IRQF_TRIGGER_RISING, "n2dm_acc_irq2", stat);
		if (err < 0) {
...
static int n2dm_proc_show(struct seq_file *m, void *v)
{
        return seq_printf(m, "0x%x\n", chip_id);
}
static int n2dm_proc_open(struct inode *inode, struct file *file)
{
        return single_open(file, n2dm_proc_show, NULL);
}

static const struct file_operations n2dm_proc_fops = {
        .open           = n2dm_proc_open,
        .read           = seq_read,
        .llseek         = seq_lseek,
...
static void create_n2dm_info_proc_file(void)
{
	n2dm_info_proc_file = proc_create("driver/accel", 0644, NULL, &n2dm_proc_fops);
	if (!n2dm_info_proc_file) 
	{
		printk(KERN_INFO "n2dm proc file create failed!\n");
	}
}

static void remove_n2dm_info_proc_file(void)
{
	if(n2dm_info_proc_file){
		remove_proc_entry("driver/accel", NULL);
		n2dm_info_proc_file = NULL;
	}
}
...
static int __init n2dm_acc_init(void)
{
	...
	ret = i2c_add_driver(&n2dm_acc_driver);

	if(0xff != chip_id)
		create_n2dm_info_proc_file();
