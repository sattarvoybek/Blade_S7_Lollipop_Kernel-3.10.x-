...
//	printk("ljs %s buffer[0]=%x\n", __func__, buffer[0]);
	if (!((buffer[0]>>4) & 0X01)) {
		if(st480->ic_package_size == 4) { // ST480_SIZE_3X3_QFN
			switch(st480->board_location_degree) {
			case 0:
				if(st480->board_location == 0) { // front
		...
		} else if(st480->ic_package_size == 3) { // ST480_SIZE_2X2_BGA
			switch(st480->board_location_degree) {
			case 0:
				if(st480->board_location == 0) { // front
					mag.mag_x = (buffer[3]<<8)|buffer[4];
		...
		} else if(st480->ic_package_size == 2) { //ST480_SIZE_1_6X1_6_LGA
			switch(st480->board_location_degree) {
			case 0:
				if(st480->board_location == 0) { // front
                                mag.mag_x = (buffer[5]<<8)|buffer[6];
		...
		} else if(st480->ic_package_size == 1) { //ST480_SIZE_1_6X1_6_BGA
			switch(st480->board_location_degree) {
			case 0: 
				if(st480->board_location == 0) { // front
                                mag.mag_x = (buffer[5]<<8)|buffer[6];
		...
		} else if(st480->ic_package_size == 0) { //ST480_SIZE_1_2X1_2
			switch(st480->board_location_degree) {
			case 0:
				if(st480->board_location == 0) { // front
                                mag.mag_x = (buffer[5]<<8)|buffer[6];
...
static void st480_input_func(struct work_struct *work)
{
	...
	st480 = container_of((struct delayed_work *)work, struct st480_data, work);
...
static ssize_t st480_board_location_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", st480->board_location);
}

static ssize_t st480_board_location_degree_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", st480->board_location_degree);
}

static ssize_t st480_package_size_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", st480->ic_package_size);
}
...
static DEVICE_ATTR(board_location, S_IRUGO, st480_board_location_show, NULL);
static DEVICE_ATTR(board_location_degree, S_IRUGO, st480_board_location_degree_show, NULL);
static DEVICE_ATTR(package_size, S_IRUGO, st480_package_size_show, NULL);
...
static struct device_attribute *sensor_attrs[] = {
	...
	&dev_attr_board_location_degree,
	&dev_attr_package_size,
	NULL,
};

static int st480_parse_dt(struct device *dev,  struct st480_data *pdata)
{
	struct device_node *np = dev->of_node;
	u32 temp_val;
	int rc;

	rc = of_property_read_u32(np, "board-location", &temp_val);
	if (rc && (rc != -EINVAL)) {
		dev_err(dev, "Unable to read board-location\n");
		return rc;
	} else {
		pdata->board_location = temp_val;
	}

	rc = of_property_read_u32(np, "board-location-degree", &temp_val);
	if (rc && (rc != -EINVAL)) {
		dev_err(dev, "Unable to read board-location-degree\n");
		return rc;
	} else {
		pdata->board_location_degree = temp_val;
	}

	rc = of_property_read_u32(np, "package-size", &temp_val);
	if (rc && (rc != -EINVAL)) {
		dev_err(dev, "Unable to read package-size\n");
		return rc;
	} else {
		pdata->ic_package_size = temp_val;
	}
	return rc;
}

static int st480_proc_show(struct seq_file *m, void *v)
{
        return seq_printf(m, "0x%x\n", chipid);
}
static int st480_proc_open(struct inode *inode, struct file *file)
{
        return single_open(file, st480_proc_show, NULL);
}

static const struct file_operations st480_proc_fops = {
        .open           = st480_proc_open,
        .read           = seq_read,
        .llseek         = seq_lseek,
...
static int st480_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (client->dev.of_node) {
		...
		if (IS_ERR(pinctrl))
			printk(KERN_ERR "pins are not configured from the driver\n");

		err = st480_parse_dt(&client->dev, st480);
		if(err) {
			 printk(KERN_ERR "Unable to parse platfrom data err=%d\n", err);
			 goto exit2;
		}
	}

	client->addr = 0x0d; // The i2c addr of ST480 and AKM09911 is the same
	st480->client = client;
   ...
   if (!proc_create("driver/compass", 0644, NULL, &st480_proc_fops)) {
        dev_err(&client->dev, "proc file create failed!");
    } 
