...
//wangxiaomei add fuction 
static void subsystem_freeup(struct subsys_device *dev, void *data)
{
	if (dev->desc->freeup)
		dev->desc->freeup(dev->desc);
	//if (dev->desc->shutdown)
	//dev->desc->shutdown(dev->desc, 0);
}
#endif
...
#if defined(CONFIG_SS_3G_DDR)
	for_each_subsys_device(list, count, NULL, subsystem_freeup);  //wangxiaomei add 
#endif
