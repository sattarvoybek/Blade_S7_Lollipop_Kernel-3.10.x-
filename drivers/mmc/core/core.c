...
{
	int err = 0;
	struct mmc_card *card = host->card;  //wangxiaomei add for hynix reduce lifetime.
	...
	{
		pr_warning("%s:wangxiaomei  hynix eMMC detected , keep power to eMMC during sleep\n",
			       mmc_hostname(host));
...
int mmc_resume_host(struct mmc_host *host)
{
	...
	if (host->bus_ops && !host->bus_dead) {
		if (!mmc_card_keep_power(host)) {
			...
			if (card != NULL && card->cid.manfid == 0x90)
		{
			pr_warning("%s: wangxiaomei hynix eMMC detected , restores clock\n",
				       mmc_hostname(host));
