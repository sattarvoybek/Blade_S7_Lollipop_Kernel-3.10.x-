...
#define MICRON_EMMC_MANUFACTURER_ID     0xFE
...
int  msm_emmc_info_read_samsung_proc(void);
 int  msm_emmc_info_read_sandisk_proc(void);
 int  msm_emmc_info_read_micron_proc(void);
 int  msm_emmc_info_read_hynix_proc(void);
...
int  msm_emmc_info_read_samsung_proc(void)
{
	...
	#if defined(CONFIG_MMC_8G)
	strcpy(emmc_module_name,"SAMSUNG 8GB eMMC");
	#elif defined(CONFIG_MMC_32G)
	strcpy(emmc_module_name,"SAMSUNG 32GB eMMC");
	#else
    strcpy(emmc_module_name,"SAMSUNG 16GB eMMC");
	#endif
  //  len = sprintf(buf, "%s\n", emmc_module_name);
    return len;
}
int msm_emmc_info_read_sandisk_proc(void)
{
    ...
    #if defined(CONFIG_MMC_8G)
	strcpy(emmc_module_name,"SANDISK 8GB eMMC");
	#elif defined(CONFIG_MMC_32G)
	strcpy(emmc_module_name,"SANDISK 32GB eMMC");
	#else
    strcpy(emmc_module_name,"SANDISK 16GB eMMC");
	#endif
  //  len = sprintf(buf, "%s\n", emmc_module_name);
    return len;
}
...
{
    int len = 0;
    printk("WJP:enter msm_emmc_info_read_micron_proc \n");
	#if defined(CONFIG_MMC_8G)
	strcpy(emmc_module_name,"MICRON 8GB eMMC");
	#elif defined(CONFIG_MMC_32G)
	strcpy(emmc_module_name,"MICRON 32GB eMMC");
	#else
    strcpy(emmc_module_name,"MICRON 16GB eMMC");
	#endif
  //  len = sprintf(buf, "%s\n", emmc_module_name);
    return len;
}
int msm_emmc_info_read_unknow_proc(void)
{
    int len = 0;
    printk("WJP:enter msm_emmc_info_read_unknow_proc \n");
    strcpy(emmc_module_name,"UNKNOW  EMMC PRODUCT ");
  //  len = sprintf(buf, "%s\n", emmc_module_name);
    return len;
}
int msm_emmc_info_read_hynix_proc(void)
{
	...
	#if defined(CONFIG_MMC_8G)
	strcpy(emmc_module_name,"HYNIX 8GB eMMC");
	#elif defined(CONFIG_MMC_32G)
	strcpy(emmc_module_name,"HYNIX 32GB eMMC");
	#else
    strcpy(emmc_module_name,"HYNIX 16GB eMMC");
	#endif
  //  len = sprintf(buf, "%s\n", emmc_module_name);
    return len;
}
void init_emmc_info_proc(struct mmc_host *host)
{
    ...
    d_entry = proc_create("driver/emmc", 0, NULL,&emmc_proc_fops);

    if (d_entry)
    {
        if (host->card->cid.manfid == SAMSUNG_EMMC_MANUFACTURER_ID)
        {
               msm_emmc_info_read_samsung_proc();
        }
        else if (host->card->cid.manfid == HYNIX_EMMC_MANUFACTURER_ID)
        {
               msm_emmc_info_read_hynix_proc();
        }
        else if (host->card->cid.manfid == SANDISK_EMMC_MANUFACTURER_ID)
        {
                msm_emmc_info_read_sandisk_proc();
        }
        else if (host->card->cid.manfid == MICRON_EMMC_MANUFACTURER_ID)
        {
                msm_emmc_info_read_micron_proc();
        }
        else  
        {
                msm_emmc_info_read_unknow_proc();
        }
       // d_entry->data = NULL;
    }
...
static int mmc_suspend(struct mmc_host *host)
{
	...
	BUG_ON(!host);
	BUG_ON(!host->card);
    pr_info("%s : wangxiaomei mmc_suspend()::enter\n",mmc_hostname(host));
...
if (card != NULL && card->cid.manfid == 0x90)
	{
		pr_warning("%s:wangxiaomei hynix eMMC detected , don't clear the eMMC state before deselect eMMC \n",
			       mmc_hostname(host));
	}
	else
	{
...
out:
	mmc_release_host(host);
	pr_info("%s : wagnxiaomei mmc_suspend()::exit , err = %d\n",mmc_hostname(host),err);
	return err;
}
...
static int mmc_resume(struct mmc_host *host)
{
	...
	BUG_ON(!host);
	BUG_ON(!host->card);
	pr_info("%s : wangxiaomei mmc_resume()::enter\n",mmc_hostname(host));
	card = host->card;
	mmc_claim_host(host);
	if (card != NULL && card->cid.manfid == 0x90)
	{
		pr_warning("%s: wangxiaomei hynix eMMC detected , using CMD7CMD5 - CMD5CMD7 insteading of CMD7CMD5-CMD0CMD1 to awake eMMC\n",
			       mmc_hostname(host));
		if(mmc_card_can_sleep(host))
		{
			...
			if(err)
			{
				pr_err("%s : wangxiaomei mmc_resume()::eMMC awake failed , err = %d\n",mmc_hostname(host),err);
				mmc_release_host(host);
				return err;
			}
		}
		...
		{		
			...
			if(err)
				pr_err("%s : wangxiaomei mmc_resume()::enable cache failed , err = %d\n",mmc_hostname(host),err);	
		}
	}
	else
	{
    ...
    pr_info("%s : wangxiaomei mmc_resume()::exit , err = %d\n",mmc_hostname(host),err);
	return err;
}
