...
int mdss_dsi_on(struct mdss_panel_data *pdata)
{
	...
	if (mipi->init_delay)
       ...
       mipi->force_clk_lane_hs = 1; //Forcing clock lanes to always HS  yangchaofeng add patch modify
...
/*ZSW_MODIFY begin,add for P829A10 LCD in KERNEL,yangchaofeng,2010512*/
    ...
    ctrl_pdata->disp_vsp_en_gpio = of_get_named_gpio(ctrl_pdev->dev.of_node,"qcom,platform-vsp-en-gpio", 0);
	if (!gpio_is_valid(ctrl_pdata->disp_vsp_en_gpio)) {
		pr_err("%s:%d, qcom,platform-vsp-en-gpio not specified\n",__func__, __LINE__);
	}

    ctrl_pdata->disp_vsn_en_gpio = of_get_named_gpio(ctrl_pdev->dev.of_node,"qcom,platform-vsn-en-gpio", 0);
	if (!gpio_is_valid(ctrl_pdata->disp_vsn_en_gpio)) {
		pr_err("%s:%d, qcom,platform-vsn-en-gpio not specified\n",__func__, __LINE__);
	}
#endif
/*ZSW_MODIFY end,add for P829A10 LCD in KERNEL,yangchaofeng,2010512*/
/*ZSW_MODIFY begin,add for P839F35 LCD in KERNEL,yangchaofeng,2010512*/
       ...
       if (!gpio_is_valid(ctrl_pdata->disp_vsp_en_gpio)) {
               pr_err("%s:%d, qcom,platform-vsp-en-gpio not specified\n",__func__, __LINE__);
                ...
                if (rc) {
                    ...
                    gpio_direction_output(ctrl_pdata->disp_vsp_en_gpio, 1);//config gpio to output
                }
        }
       ...
       if (!gpio_is_valid(ctrl_pdata->disp_vsn_en_gpio)) {
               pr_err("%s:%d, qcom,platform-vsn-en-gpio not specified\n",__func__, __LINE__);
                ...
                if (rc) {
                    ...
                    gpio_direction_output(ctrl_pdata->disp_vsn_en_gpio, 1);//config gpio to output
                }
        }
#endif
/*ZSW_MODIFY end,add for P839F35 LCD in KERNEL,yangchaofeng,2010512*/
