...
struct mdss_dsi_ctrl_pdata {
    ...
    int(*ce)(struct mdss_panel_data *pdata, int ce_mode);
    #endif
    //zhangjian add end
...
/*ZSW_MODIFY end,add for P839F35 LCD in KERNEL,yangchaofeng,2010512*/
	...
	struct dsi_panel_cmds on_cmds_wakeup_gesture;
	struct dsi_panel_cmds off_cmds_wakeup_gesture;
	#endif
	//add end
   //zhangjian add for ce
    #ifdef CONFIG_LCD_DISPLAY_ENHANCE
	struct dsi_panel_cmds CEdefault_cmds; 
	struct dsi_panel_cmds CEbright_cmds; 
    #endif 
	//zhangjian add end
...
extern void Glove_mod_enter_deppidle(void);
#endif
//add end
#ifdef CONFIG_TP_WAKEUP_GESTURE
extern bool lcd_get_tsc_gesture_state(void);
/*ZSW_MODIFY begin,add for get tp_gesture status in KERNEL,yangchaofeng,2010610*/
extern bool lcd_get_sharp_gesture_state(void);
#ifdef  CONFIG_ZTE_LCD_P839F35
#define lcd_get_gesture_state lcd_get_sharp_gesture_state
#else
#define lcd_get_gesture_state lcd_get_tsc_gesture_state
#endif
