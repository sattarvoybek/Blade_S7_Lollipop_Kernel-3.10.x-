...
static inline bool mdss_dsi_poll_clk_lane(struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	if (readl_poll_timeout(((ctrl->ctrl_base) + 0x00a8),
				clk,
				(clk & 0x0010),
				100, 20000)) {    // 10, 1000 yangchaofeng modify for add patch:7c8d8f 20150713
...
static void mdss_dsi_stop_hs_clk_lane(struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	mdss_dsi_clk_ctrl(ctrl, DSI_ALL_CLKS, 1);
/*ZSW_MODIFY end,add display patch(1295980&1295981) for some error logs,yangchaofeng,20150623*/
	/* fifo */
	if (readl_poll_timeout(((ctrl->ctrl_base) + 0x000c),
			   fifo,
			   ((fifo & 0x11110000) == 0x11110000),
			       100, 50000)) {  // 10, 1000 yangchaofeng modify for add patch:7c8d8f 20150713
	...
	if (readl_poll_timeout(((ctrl->ctrl_base) + 0x00a8),
			   lane,
			   ((lane & 0x000f) == 0x000f),
			       100, 20000)) {   // 100, 2000 yangchaofeng modify for add patch:7c8d8f 20150713
...
int mdss_dsi_cmdlist_commit(struct mdss_dsi_ctrl_pdata *ctrl, int from_mdp)
{
    ...
    //if (req->flags & CMD_REQ_HS_MODE) //zhangjian delete
        #if (defined CONFIG_ZTE_LCD_P839F35) ||(defined CONFIG_ZTE_NOT_INCELL_LCD) ||(defined CONFIG_ZTE_LCD_P829A10)
