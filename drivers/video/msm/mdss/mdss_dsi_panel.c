...
/*ZSW_MODIFY begin,add for P839F35 LCD in KERNEL,yangchaofeng,2010520*/
#ifdef CONFIG_ZTE_LCD_P839F35
extern int tps65132_set_vsp_vsn_58v(void);
int lcm_vsp_vsn_58v = 0x0;
#endif
/*ZSW_MODIFY end,add for P839F35 LCD in KERNEL,yangchaofeng,2010520*/
...
static struct dsi_cmd_desc display_off_CABC_backlight_cmds[] = {
  {{DTYPE_DCS_LWRITE, 1, 0, 0, 2, sizeof(CABC_0x51_off)}, CABC_0x51_off},
	{{DTYPE_DCS_LWRITE, 1, 0, 0, 2, sizeof(CABC_0x53_off)}, CABC_0x53_off }
	};

static char CABC_0x51[2]={0x51,0xff};
/*begin ,yangchaofeng add for F30-LCD*/
#if (defined CONFIG_ZTE_NOT_INCELL_LCD)
static char CABC_0x53[2]={0x53,0x24};
#else
static char CABC_0x53[2]={0x53,0x2c};
#endif
static struct dsi_cmd_desc CABC_backlight_cmds[] = {
	{{DTYPE_DCS_LWRITE, 1, 0, 0, 2, sizeof(CABC_0x51)}, CABC_0x51},
	{{DTYPE_DCS_LWRITE, 1, 0, 0, 2, sizeof(CABC_0x53)}, CABC_0x53}
};
#endif
//modify end
...
#endif 
//add end
/*ZSW_MODIFY begin,add for P829A10 LCD in KERNEL,yangchaofeng,2010512*/
#ifdef CONFIG_ZTE_LCD_P829A10
int mdss_dsi_panel_power_enable(struct mdss_panel_data *pdata, int enable)
{
    struct mdss_dsi_ctrl_pdata *ctrl_pdata = NULL;
	struct mdss_panel_info *pinfo = NULL;
	...
	if (enable)
	{
	            ...
	            if (gpio_is_valid(ctrl_pdata->disp_vsp_en_gpio)) {
                            rc = gpio_request(ctrl_pdata->disp_vsp_en_gpio,"disp_vsp_en_gpio");
                            if (rc)
			 	{
	        	                pr_err("request disp_vsp_en_gpio failed, rc=%d\n", rc);
	                      }
		                gpio_value = gpio_get_value((unsigned int)(ctrl_pdata->disp_vsp_en_gpio));
		                gpio_direction_output(ctrl_pdata->disp_vsp_en_gpio, gpio_value);//config gpio to output
		                pr_err(LCD_DEVICE"%s: enable disp_vsp_en_gpio \n", __func__);
		                gpio_set_value((ctrl_pdata->disp_vsp_en_gpio), 1);
		                msleep(2);
	        	}
	
	            if (gpio_is_valid(ctrl_pdata->disp_vsn_en_gpio)) {
	        		rc = gpio_request(ctrl_pdata->disp_vsn_en_gpio,"disp_vsn_en_gpio");
	        		if (rc) 
				{
	        			pr_err("request disp_vsn_en_gpio failed, rc=%d\n", rc);
	        		}
		                gpio_value = gpio_get_value((unsigned int)(ctrl_pdata->disp_vsn_en_gpio));
		                gpio_direction_output(ctrl_pdata->disp_vsn_en_gpio, gpio_value);//config gpio to output
		                pr_err(LCD_DEVICE"%s: enable disp_vsn_en_gpio \n", __func__);
		                gpio_set_value((ctrl_pdata->disp_vsn_en_gpio), 1);
		                msleep(2);
	        	}

	            gpio_free(ctrl_pdata->disp_vddio_en_gpio);
	            gpio_free(ctrl_pdata->disp_vdddc_en_gpio);
	            gpio_free(ctrl_pdata->disp_vsn_en_gpio);
	            gpio_free(ctrl_pdata->disp_vsp_en_gpio);
        }
	else
	{
	            if (gpio_is_valid(ctrl_pdata->disp_vsn_en_gpio)) {
	        		...
	        		if (rc) 
				{
		                ...
		                gpio_value = gpio_get_value((unsigned int)(ctrl_pdata->disp_vsn_en_gpio));
		                gpio_direction_output(ctrl_pdata->disp_vsn_en_gpio, gpio_value);//config gpio to output
		                pr_err(LCD_DEVICE"%s: disable disp_vsn_en_gpio \n", __func__);
		                gpio_set_value((ctrl_pdata->disp_vsn_en_gpio), 0);
		                msleep(2);
	        	}
	            ...
	            if (gpio_is_valid(ctrl_pdata->disp_vsp_en_gpio)) {
                            ...
                            if (rc)
			 	{
		                ...
		                gpio_value = gpio_get_value((unsigned int)(ctrl_pdata->disp_vsp_en_gpio));
		                gpio_direction_output(ctrl_pdata->disp_vsp_en_gpio, gpio_value);//config gpio to output
		                pr_err(LCD_DEVICE"%s: disable disp_vsp_en_gpio \n", __func__);
		                gpio_set_value((ctrl_pdata->disp_vsp_en_gpio), 0);
		                msleep(2);
	        	}
	            ...
	            if (gpio_is_valid(ctrl_pdata->disp_vddio_en_gpio)) {
	        		...
	        		if (rc) {
		                ...
		                gpio_value = gpio_get_value((unsigned int)(ctrl_pdata->disp_vddio_en_gpio));
		                gpio_direction_output(ctrl_pdata->disp_vddio_en_gpio, gpio_value);//config gpio to output
		                pr_err(LCD_DEVICE"%s: disable disp_vddio_en_gpio \n", __func__);
	            ...
	            if (gpio_is_valid(ctrl_pdata->disp_vdddc_en_gpio)) {
	        		...
	        		if (rc) {
		                ...
		                gpio_value = gpio_get_value((unsigned int)(ctrl_pdata->disp_vdddc_en_gpio));
		                gpio_direction_output(ctrl_pdata->disp_vdddc_en_gpio, gpio_value);//config gpio to output
		                pr_err(LCD_DEVICE"%s: disable disp_vdddc_en_gpio \n", __func__);
...
#endif
/*ZSW_MODIFY end,add for P829A10 LCD in KERNEL,yangchaofeng,2010512*/
...
int mdss_dsi_panel_power_enable(struct mdss_panel_data *pdata, int enable)
{
    ...
    if (enable)
    {
    pr_err(LCD_DEVICE"%s: enable disp_power_gpio in F35 \n", __func__);
    gpio_set_value((ctrl_pdata->disp_vddio_en_gpio), 1);
    msleep(5);
    ...
    {
    pr_err(LCD_DEVICE"%s: disable disp_power_gpio in F35 \n", __func__);
...
int mdss_dsi_panel_reset(struct mdss_panel_data *pdata, int enable)
{
	...
	if (enable) {
      ...
      //zhangjian modify
      #if (defined CONFIG_TP_WAKEUP_GESTURE) &&  !(defined CONFIG_ZTE_LCD_P839F35)
      if (!lcd_get_gesture_state())
      {
		  pr_err(LCD_DEVICE"%s: lcd_get_gesture_state is 0 \n", __func__);
      ...
      #elif (defined CONFIG_TP_WAKEUP_GESTURE) &&  (defined CONFIG_ZTE_LCD_P839F35)
               pr_err(LCD_DEVICE"%s:enable  reset-pin state: %s\n",__func__,lcd_get_gesture_state()?"tp_gesture_enable":"tp_gesture_disable");//add by yangchaofeng for debug lcd
               for (i = 0; i < pdata->panel_info.rst_seq_len; ++i) {
				gpio_set_value((ctrl_pdata->rst_gpio),pdata->panel_info.rst_seq[i]);
				if (pdata->panel_info.rst_seq[++i])
					usleep(pinfo->rst_seq[i] * 1000);
      ...
      #else
               pr_err(LCD_DEVICE"%s:enable reset-pin no_tp_gesture\n",__func__);//add by yangchaofeng for debug lcd
               for (i = 0; i < pdata->panel_info.rst_seq_len; ++i) {
      ...
      //end modify
		...
		//zhangjian add for 8939
       /*ZSW_MODIFY begin,add for P839F35 LCD in KERNEL,yangchaofeng,2010512*/
           #if (defined CONFIG_ZTE_LCD_P839F35) || (defined CONFIG_ZTE_LCD_P829A10) ||(defined CONFIG_ZTE_NOT_INCELL_LCD)
           #else	
               gpio_free(ctrl_pdata->rst_gpio);
           #endif
       /*ZSW_MODIFY end,add for P839F35 LCD in KERNEL,yangchaofeng,2010512*/
       ...
       #if (defined CONFIG_ZTE_NOT_INCELL_LCD) || (defined CONFIG_ZTE_LCD_P829A10)
                printk(LCD_DEVICE"%s: disable reset-pin \n",__func__);
...
/*ZSW_MODIFY begin,add for get tp_gesture status in KERNEL,yangchaofeng,2010610*/
                ...
                if(lcd_get_gesture_state())
                {
                        printk(LCD_DEVICE"%s:disable set reset-pin F35 to 1\n",__func__);
                        //gpio_set_value((ctrl_pdata->rst_gpio), 0);
                        gpio_free(ctrl_pdata->rst_gpio);
                }
                else
                {      
                        printk(LCD_DEVICE"%s:disable set reset-pin F35 to 0\n",__func__);
                        gpio_set_value((ctrl_pdata->rst_gpio), 0);
                        gpio_free(ctrl_pdata->rst_gpio);
                }
        #else
...
static int mdss_dsi_panel_on(struct mdss_panel_data *pdata)
{
  ...
  {
	   pr_err(LCD_DEVICE"%s: ctrl->on_cmds.cmd_cnt is %d\n", __func__,ctrl->on_cmds.cmd_cnt);
	   if (ctrl->on_cmds.cmd_cnt)
		    mdss_dsi_panel_cmds_send(ctrl, &ctrl->on_cmds);
  }
  #else
  pr_err(LCD_DEVICE"%s: on_cmds no_tp_gesture\n",__func__);//add by yangchaofeng for debug lcd
  if (ctrl->on_cmds.cmd_cnt)
		    mdss_dsi_panel_cmds_send(ctrl, &ctrl->on_cmds);
  #endif
  //end modify
...
static int mdss_dsi_panel_off(struct mdss_panel_data *pdata)
{
  ...
  if (lcd_get_gesture_state())
  {
      //zhangjian add for sharp TP
	  #if defined(CONFIG_TOUCHSCREEN_SHTSC_I2C)
      Glove_mod_enter_deppidle();
	  //pr_err(LCD_DEVICE"%s:Glove_mod_enter_deppidle\n",__func__);
      #endif
	  //add end
     pr_err(LCD_DEVICE"%s:ctrl->off_cmds_wakeup_gesture\n",__func__);
     if (ctrl->off_cmds_wakeup_gesture.cmd_cnt)
		    mdss_dsi_panel_cmds_send(ctrl, &ctrl->off_cmds_wakeup_gesture);
  }
  else
  {
           pr_err(LCD_DEVICE"%s:ctrl->off_cmds\n",__func__);
	    if (ctrl->off_cmds.cmd_cnt)
		     mdss_dsi_panel_cmds_send(ctrl, &ctrl->off_cmds);
  }
  #else
   pr_err(LCD_DEVICE"%s:off_cmds no_tp_gesture\n",__func__);//add by yangchaofeng for debug lcd
   if (ctrl->off_cmds.cmd_cnt)
		     mdss_dsi_panel_cmds_send(ctrl, &ctrl->off_cmds);
  #endif
  //end modify
