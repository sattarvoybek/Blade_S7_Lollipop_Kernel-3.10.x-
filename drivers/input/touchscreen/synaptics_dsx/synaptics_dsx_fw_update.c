...
#define FW_IMAGE_NAME_JDI "dsx_jdi_4291.img"

#define FW_IMAGE_NAME_JDI_F30	"PR1808273_44463149.img"
static char fwname[256];
//char *syna_file_name;
//char  *syna_fw_name=FW_IMAGE_NAME_JDI;
#define NAME_BUFFER_SIZE 256

#define DSX_UP(fmt,arg...)		   printk("[TP:UP]"fmt"\n",##arg)
...
static ssize_t syna_doreflash_read(struct file *file,char __user *buffer, size_t count, loff_t *offset);

static ssize_t syna_doreflash_write(struct file *filp,const char *buff, size_t len,loff_t * off);
...
static ssize_t syna_force_fwupdate_show(struct device *dev,
...
static struct device_attribute attrs[] = {
	__ATTR(forcereflash, /*S_IWUGO*/ (S_IRUGO | S_IWUGO),
			synaptics_rmi4_show_error,
			fwu_sysfs_do_reflash_store),
	__ATTR(doreflash, /*S_IWUGO*/ (S_IRUGO | S_IWUGO),
			syna_force_fwupdate_show,
			syna_force_fwupdate_store),
	__ATTR(writeconfig, /*S_IWUGO*/ (S_IRUGO | S_IWUGO),
			synaptics_rmi4_show_error,
			fwu_sysfs_write_config_store),
	__ATTR(readconfig, /*S_IWUGO*/ (S_IRUGO | S_IWUGO),
			synaptics_rmi4_show_error,
			fwu_sysfs_read_config_store),
	__ATTR(configarea, /*S_IWUGO*/ (S_IRUGO | S_IWUGO),
			synaptics_rmi4_show_error,
			fwu_sysfs_config_area_store),
	__ATTR(imagename, /*S_IWUGO*/ (S_IRUGO | S_IWUGO),
			synaptics_rmi4_show_error,
			fwu_sysfs_image_name_store),
	__ATTR(imagesize, /*S_IWUGO*/ (S_IRUGO | S_IWUGO),
			synaptics_rmi4_show_error,
			fwu_sysfs_image_size_store),
	__ATTR(blocksize, /*S_IRUGO*/ (S_IRUGO | S_IWUGO),
			fwu_sysfs_block_size_show,
			synaptics_rmi4_store_error),
	__ATTR(fwblockcount, /*S_IRUGO*/ (S_IRUGO | S_IWUGO),
			fwu_sysfs_firmware_block_count_show,
			synaptics_rmi4_store_error),
	__ATTR(configblockcount, /*S_IRUGO*/ (S_IRUGO | S_IWUGO),
			fwu_sysfs_configuration_block_count_show,
			synaptics_rmi4_store_error),
	__ATTR(permconfigblockcount, /*S_IRUGO*/ (S_IRUGO | S_IWUGO),
			fwu_sysfs_perm_config_block_count_show,
			synaptics_rmi4_store_error),
	__ATTR(blconfigblockcount, /*S_IRUGO*/ (S_IRUGO | S_IWUGO),
			fwu_sysfs_bl_config_block_count_show,
			synaptics_rmi4_store_error),
	__ATTR(dispconfigblockcount, /*S_IRUGO*/ (S_IRUGO | S_IWUGO),
...
static int fwu_read_f34_queries(void)
{
	...
	if (fwu->bootloader_id[1] == '5') {
		...
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Unrecognized bootloader version :%c\n",
...
//	char *firmware_id;
	...
	DSX_UP("%s: Image firmware ID = %d\n",__func__, (unsigned int)image_fw_id);

	if (image_fw_id > device_fw_id) {
		flash_area = UI_FIRMWARE;
		goto exit;
	} else if (image_fw_id < device_fw_id) {
		DSX_UP(	"%s: Image firmware ID older than device firmware ID\n",
				__func__);
		flash_area = NONE;
		goto exit;
	}
	...
	DSX_UP(	"%s: Device config ID = 0x%02x 0x%02x 0x%02x 0x%02x\n",
			__func__,
			config_id[0],
			config_id[1],
			config_id[2],
	...
	DSX_UP(	"%s: Image config ID = 0x%02x 0x%02x 0x%02x 0x%02x\n",
			__func__,
			fwu->config_data[0],
			fwu->config_data[1],
			fwu->config_data[2],
	...
	if (image_config_id > device_config_id || fwu->force_update_config) {
...
exit:
	if (flash_area == NONE) {
		DSX_UP(	"%s: No need to do reflash\n",
				__func__);
	} else {
		DSX_UP(	"%s: Updating %s\n",
				__func__,
...
static int fwu_do_reflash(void)
{
	...
	if (fwu->config_data) {
		...
		if (retval < 0)
			return retval;
		dev_info(rmi4_data->pdev->dev.parent,"%s: Configuration programmed\n", __func__);
	}
      DSX_UP("Enter %s@%d\n", __func__,__LINE__);
	if (fwu->disp_config_data) {
		...
		if (retval < 0)
			return retval;
		dev_info(rmi4_data->pdev->dev.parent,"%s: Display configuration programmed\n", __func__);
	}
      DSX_UP("Exit %s@%d\n", __func__,__LINE__);
	return retval;
}
...
static int fwu_do_write_config(void)
{
	...
	dev_dbg(rmi4_data->pdev->dev.parent,
			"%s: Bootloader ID written\n",
			__func__);
      DSX_UP("Enter %s@%d ,fwu->config_area=%d\n", __func__,__LINE__,fwu->config_area);
...
int fwu_start_reflash(void)
{
	...
	char *firmware_name=NULL; 
		...
		DSX_UP("Enter %s@%d: ", __func__,__LINE__);
	...
	//return 0;
	/***zhangzhao ****/
	//fwu->ext_data_source = 0;
	//fwu->productinfo1 =1;
	/***zhangzhao ****/
	if (fwu->ext_data_source) {
		fw_image = fwu->ext_data_source;
		DSX_UP("Enter %s@%d: Start of reflash process\n", __func__,__LINE__);
	} else {

	if(fwname[0]==0)
		{
		  // auto_update_flag = 1;
                DSX_UP("Enter %s@%d: Start of reflash process,fwu->productinfo1=0x%x", __func__,__LINE__,fwu->productinfo1);			
           	switch (fwu->productinfo1){
               case 0x01:
                   DSX_UP("%s:jdi\n",__func__);
           		firmware_name = FW_IMAGE_NAME_JDI;
           		break;
              	case 0x1f:
                   DSX_UP("%s:tpk\n",__func__);
           	...
           	case 0x27:
           		...
           		break;
           	 default:
                   DSX_UP("Exit %s:not support 0x%04x product\n",__func__,fwu->productinfo1);
         			retval = -EINVAL;
       			goto exit;
           	}
	}		
		...
		{
		DSX_UP("1111syna fwfile name:%s\n",fwname);


		if(!strcmp(fwname,""))
		{
			DSX_UP("%s file_name is null\n",__func__);
         			retval = -EINVAL;
       			goto exit;
		}


		//snprintf(firmware_name, NAME_BUFFER_SIZE, "%s", syna_file_name);
		//strncpy(firmware_name, syna_file_name, MAX_IMAGE_NAME_LEN);
		firmware_name = fwname;
		DSX_UP("2222syna firmware_name name:%s\n",firmware_name);

	}
		
				
		strncpy(fwu->image_name, firmware_name, MAX_IMAGE_NAME_LEN);
		DSX_UP(	"%s: 1===1Requesting firmware image %s\n",__func__, fwu->image_name);
	...
	/*	if(auto_update_flag)
			{
        		if (retval != 0) 
			{
           		firmware_name = FW_IMAGE_NAME_JDI_F30;
        		strncpy(fwu->image_name, firmware_name, MAX_IMAGE_NAME_LEN);
        		DSX_UP(	"%s: 2====2second Requesting firmware image %s\n",__func__, fwu->image_name);				
		...
		DSX_UP("%s:requeset Firmware  ok ,image size = %Zd\n",__func__, fw_entry->size);
	...
	DSX_UP("%s,ic version is %x, and image version is %x\n",__func__,fwu->bl_version,header.bootloader_version);
	if (fwu->bl_version != header.bootloader_version) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Bootloader version mismatch\n",
				__func__);
		retval = -EINVAL;
		DSX_UP("Exit %s@%d error\n", __func__,__LINE__);
		goto exit;
	}
...
exit:
	...
	DSX_UP("Exit %s@%d End of reflash process\n", __func__,__LINE__);
...
EXPORT_SYMBOL(synaptics_fw_updater_by_tpd);
...
char  *syna_fw_name="PR1780950-td4291-jdi-ScannerDrive_5P5HD_ZTE_44463044.img";
static char flag_update_firmware ;
							...
							const char *buf, size_t count)
{
	...
	DSX_UP("%s,fwname[0] is %c@%d\n",__func__,fwname[0],__LINE__);

	if ('1' == fwname[0]) 
		{
			memset(fwname, 0, sizeof(fwname));
			//sprintf(fwname, "%s", syna_fw_name);
		      strncpy(fwname, syna_fw_name, MAX_IMAGE_NAME_LEN);
		}

	//fwname[count-1] = '\0';

	DSX_UP("%s,firmware name  is %s@%d\n",__func__,fwname,__LINE__);
	...
	if(0 == fwu_start_reflash())
		{
		DSX_UP("%s: update success \n", __func__);
		flag_update_firmware = '2';
		}
	else
		{
		DSX_UP("%s: update fail  \n", __func__);
...
#if defined (CONFIG_CREATE_SYS_INTERFACE) && defined (CONFIG_TPD_AUTO_UPDATE_FIRMWARE)
...
void synaptics_create_auto_update_fw_work(void)
{
	...
	if (!tpd_firmware_upgrade) {
		DSX_UP("Error Could not create work queue tpd_firmware_upgrade: no memory");
	}
...
static ssize_t syna_doreflash_read(struct file *file,char __user *buffer, size_t count, loff_t *offset)
{
	uint8_t buffer_tpd[10];
	ssize_t len = 0;

	//len =  snprintf(buf, PAGE_SIZE, "%c%c%c\n", 'o','k',flag_update_firmware);
	len += sprintf(buffer_tpd+len, "%c%c%c\n", 'o','k',flag_update_firmware);
...
//	DSX_UP("%s,messages[0] is %c@%Zd\n",__func__,messages[0],strlen(messages));
	DSX_UP("%s,fwname is %s@%Zd\n",__func__,fwname,strlen(fwname));
	...
	if ('1' == fwname[0]) {
		...
		{
		//sprintf(fwname, "%s", messages);
		//fwname[strlen(messages)-1] = '\0';
		
		//strncpy(fwname, messages, MAX_IMAGE_NAME_LEN);
	}

	DSX_UP("%s,fwname[0] is %c@%Zd\n",__func__,fwname[0],strlen(fwname));
	fwu->force_update = true;
    ...
    static const struct file_operations tsc_doreflash = {
        .owner = THIS_MODULE,
        .read = syna_doreflash_read,
        .write = syna_doreflash_write,
    };

#if 0
/*zhangzhao add for power off charger no register tsc driver*/
#define	ANDROID_BOOT_MODE_MAX	30
static char phone_mode[ANDROID_BOOT_MODE_MAX];

static int __init phone_mode_detect(char *str) 
{
	//int i;
	//get_options(str, ARRAY_SIZE(phone_mode), phone_mode);
	strlcpy(phone_mode, str, ANDROID_BOOT_MODE_MAX);
	
	//for(i=0;i<8;i++)
		printk("ZZ******%s\n",phone_mode);
	return 0;
}
__setup("androidboot.mode=", phone_mode_detect);
/*zhangzhao add for power off charger no register tsc driver*/
#endif
...
static int synaptics_rmi4_fwu_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	DSX_UP("productinfo1 is %d,productinfo2 is %d\n",fwu->productinfo1,fwu->productinfo2);
	...
	DSX_UP("tpd:  it is synaptics_rmi4_fwu_init OK --1\n");
	...
	queue_work(fwu->fwu_workqueue,
			&fwu->fwu_work);
		DSX_UP("tpd:  it is synaptics_rmi4_fwu_init OK ---2\n");

#endif
#if defined (CONFIG_CREATE_SYS_INTERFACE) && defined (CONFIG_TPD_AUTO_UPDATE_FIRMWARE)
//     if((phone_mode !=NULL)&&(!strcmp(phone_mode, "ffbm-01")))
//     	{
//	DSX_UP("tpd:  ZCORE mode cant update firmware\n");
//     	}
//	 else
//	 {
	synaptics_create_auto_update_fw_work();
	DSX_UP("tpd:  it is synaptics_rmi4_fwu_init OK ----3\n");
//	 }

#endif

	mt_entry = proc_create("driver/tsc_doreflash", 0644, NULL, &tsc_doreflash);
	if (mt_entry)
		{
       	    DSX_UP("create tsc_id success\n");
		}
	else
	    DSX_UP("create tsc_id fail\n");
