...
extern s32 gtp_read_version(struct goodix_ts_data *ts, u16* version);//zhangjian modify s32 gtp_read_version(struct i2c_client *, u16* );
...
static s32 gup_init_panel(struct goodix_ts_data *ts)
{
    ...
    gtp_read_version(ts, &version);//zhangjian modify  gtp_read_version(ts->client, &version)
    msleep(10);
    return 0;
}
