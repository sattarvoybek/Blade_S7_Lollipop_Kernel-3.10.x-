...
//zhangjian add
#if 0
int fts_write_reg_count(struct i2c_client *client, u8 addr, const u8 val,u8 count)
{
    int ret = 0;
    u8 i;
    for (i = 0; i < count; i++)    
    {
        ret = fts_write_reg(client, addr, val);
		printk( "fts_write_reg_count ret is %d\n",ret);
        if (ret > 0)
        {
            return 0;
        }
	...
	printk( "fts_write_reg_count addr %x error\n",addr);
    return 1;
}
#endif
static int FTS_i2c_test(struct i2c_client *client)
{
  
	u8 retry=0;
	u8 reg_addr = FTS_REG_ID;
    u8 chip_id = 0;
	while(retry++ < 5)
    {
        fts_i2c_read(client, &reg_addr, 1, &chip_id, 1);
        if (chip_id == 0x54)
        {		
		    return chip_id;
        }
		printk("FTS: i2c test failed time %d.",retry);
        msleep(10);
    }
	printk("fts: chip_id is %x\n",chip_id);
	return 0;
}
//add end
...
static irqreturn_t fts_ts_interrupt(int irq, void *dev_id)
{
	...
	//disable_irq_nosync(fts_ts->client->irq); //zhangjian delete
...
static int fts_ts_start(struct device *dev)
{
   ...
   if (!data->wake_gesture_enter )//zhangjian add
   {
    ...
    {
        disable_irq_wake(data->client->irq);  //zhangjian add
    }
...
int fts_ts_suspend(struct device *dev)
{
	...
	#else
	int ret = 0;
    data->wake_gesture_enter = false;
    if ( data->wake_gesture_enable )
    {
        data->wake_gesture_enter = true;
        ret =  fts_write_reg(fts_i2c_client, 0xd0, 0x01);
		printk( "fts_write_reg ret is %d.\n",ret);
        if (ret < 0)
        {
            fts_write_reg(fts_i2c_client, 0xd0, 0x01);
			printk( "fts_write_reg 0xd0 write again.\n");
        }
        ret =  fts_write_reg(fts_i2c_client, 0xd1, 0xff);
        if (ret < 0)
        {
            fts_write_reg(fts_i2c_client, 0xd1, 0xff);
			printk( "fts_write_reg ret 0xd1 write again.\n");;
        }
        enable_irq_wake(data->client->irq);  //zhangjian add
        data->suspended = true;
        return 0;
    }	
	#endif
...
//zhangjian add
static ssize_t fts_tpd_read(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
    int len = 0;
	...
	u8 fts_fm_ver = 0;
	u8 vendor_id = 0;
	fts_read_reg(fts_i2c_client, FTS_REG_FW_VENDOR_ID, &vendor_id);
    fts_read_reg(fts_i2c_client, FTS_REG_FW_VER, &fts_fm_ver);

	printk("vendor_id is %d\n ",vendor_id);
	if (vendor_id == 0x5F)
    {        
		len += sprintf(buffer_tpd+len, "TP module: yushun\n");
    }
    else if (vendor_id == 0x80 )
    {      	
        len += sprintf(buffer_tpd+len, "TP module: yeji\n");
    }
	#endif	
...
static const struct file_operations fts_tpd_id = {
        .owner = THIS_MODULE,
        .read = fts_tpd_read,
        .write = NULL,
    };
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    #else
    chip_id = FTS_i2c_test(client);
    if (chip_id != 0x54)
    {
        printk("FTS: i2c test failed ");
		goto free_gpio;
    }
    dev_info(&client->dev, "Device ID = 0x%x\n", chip_id);
    dev_err(&client->dev, "enter fts_ts_probe 6\n");
    ...
    mt_entry = proc_create("driver/ts_information", 0644, NULL, &fts_tpd_id);
    if (mt_entry)
    {
