...
#include <sound/tlv.h>
//weiguohua add it temp
#include <linux/of_gpio.h>
//weiguohua add it temp
#include "ak4375.h"
//weiguohua add it 
#define ZTE_HIFI_STATE_PROC
#ifdef ZTE_HIFI_STATE_PROC
#include <linux/proc_fs.h>
#include <linux/seq_file.h>
#endif
//weiguohua add it 
#define AK4375_DEBUG			//used at debug mode  //weiguohua open the macro temp
//#define AK4375_CONTIF_DEBUG		//used at debug mode
//#define PLL_MICK_MODE
...
#ifdef ZTE_HIFI_STATE_PROC
static int hifi_chip_state = 0;

static int hifi_chip_show(struct seq_file *m, void *v)
{
	return	seq_printf(m, "%d\n",hifi_chip_state);
}

static int hifi_chip_open(struct inode *inode, struct file *file)
{
	return single_open(file, hifi_chip_show, NULL);
}

static const struct file_operations hifi_chip_operations = {
	.open		= hifi_chip_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
...
#ifdef AK4375_DEBUG
	SOC_ENUM_EXT("AKM Reg Read", ak4375_enum[0], get_test_reg, set_test_reg),
#endif

};
#if 0   //weiguohua add it begin
...
static int ak4375_set_pllblock(struct snd_soc_codec *codec, int fs)
{
 ...
 akdbgprt("\t[AK4375] %s nPLLClk =%d nRefClk=%d\n",__FUNCTION__, nPLLClk, nRefClk);
	...
	PLMbit--;
	MDIVbit--;
	akdbgprt("\t[AK4375] %s PLDbit =%d PLMbit=%d\n",__FUNCTION__, PLDbit, PLMbit);
	//PLD15-0
	snd_soc_write(codec, AK4375_0F_PLL_REF_CLK_DIVIDER1, ((PLDbit & 0xFF00) >> 8));
	snd_soc_write(codec, AK4375_10_PLL_REF_CLK_DIVIDER2, ((PLDbit & 0x00FF) >> 0));
	//PLM15-0
	...
	ak4375_writeMask(codec, AK4375_0E_PLL_CLK_SOURCE_SELECT, 0x01, 0x00);	//PLS=0(MICK)
#else
	snd_soc_write(codec, AK4375_13_SRC_CLK_SOURCE, (nTemp|0x01));		//DIV=0or1,SRCCKS=1(SRC Clock Select=PLL) set
...
static int ak4375_hw_params_set(struct snd_soc_codec *codec, int nfs1)
{
	...
	akdbgprt("\t[AK4375] %s(%d) nfs1=%d\n",__FUNCTION__,__LINE__,nfs1);
	...
	fs &= ~AK4375_FS;
	akdbgprt("\t[AK4375] %s(%d) fs=%d\n",__FUNCTION__,__LINE__,fs);
...
static int ak4375_probe(struct snd_soc_codec *codec)
{
	...
	akdbgprt("\t[AK4375] %s(%d) ak4375=%x\n",__FUNCTION__,__LINE__, 0);//wiguohua modify for warning

	ak4375_init_reg(codec);
	#ifdef ZTE_HIFI_STATE_PROC
		reg_hifi_state = snd_soc_read(codec, AK4375_0B_LCH_OUTPUT_VOLUME);	
	if(reg_hifi_state == 0x19){
     hifi_chip_state = 1;
	akdbgprt("\t[AK4375] hihichip  i2c read OK %s(%d)\n",__FUNCTION__,__LINE__);
       }
	#endif	
...
{
	ak4375_set_bias_level(codec, SND_SOC_BIAS_OFF);
	//gpio_direction_output(ak4375_data->gpio_reset, 0);  //weiguohua add it begin
...
static int ak4375_resume(struct snd_soc_codec *codec)
{
	...
	//gpio_direction_output(ak4375_data->gpio_reset, 1);  //weiguohua add it begin
	msleep(5);		
	ak4375_init_reg(codec);
	akdbgprt("\t[AK4375] %s(%d)\n",__FUNCTION__,__LINE__);
	return 0;
...
//weiguohua add it temp
static int akm4375_parse_dt(struct device *dev,  struct ak4375_priv *pdata)
{
        struct device_node *np = dev->of_node;
        u32 gpio_rst;
        u32 gpio_avdd_en;
	gpio_rst = of_get_named_gpio(np, "akm4375,gpio-rstn", 0);
	if (gpio_rst < 0) {
		dev_err(dev, "Unable to read gpio_rstn\n");
	} else {
		pdata->gpio_reset = gpio_rst;
	}

	gpio_avdd_en = of_get_named_gpio(np, "akm4375,gpio-avdd", 0);
	if (gpio_avdd_en < 0) {
		dev_err(dev, "Unable to read gpio-avdd-en\n");
	} else {
		pdata->gpio_avdd_enable = gpio_avdd_en;
		dev_err(dev, " read gpio-avdd-en is %d\n",pdata->gpio_avdd_enable);		
	}
	
        return 0;
}
//weiguohu add it temp
                            ...
                            const struct i2c_device_id *id)
{
	...
	#endif	
	akdbgprt("\t[AK4375] probe enter %s(%d)\n",__FUNCTION__,__LINE__);
	...
	//weiguohua add it temp
dev_set_name(codec->dev, "%s", "ak4375");
	
        if (i2c->dev.of_node) {
                printk("akm4375 use device tree\n");

                ret= akm4375_parse_dt(&i2c->dev, ak4375_data);
                if (ret) {
                        dev_err(&i2c->dev,
                ...
                printk("akm4375 not use device tree\n");                
        }

        if (gpio_is_valid(ak4375_data->gpio_avdd_enable)) {
                ret= gpio_request(ak4375_data->gpio_avdd_enable, "akm4375_avdd_en");
                if (ret) {
                        dev_err(&i2c->dev, "akm4375 avdd enable request failed");
                  return ret;
                }
			gpio_direction_output(ak4375_data->gpio_avdd_enable, 1);
			msleep(10);	
			
        }
	else{
                dev_err(&i2c->dev, "akm4375 avdd enable  gpio dt is  invalid");
	}

		
        if (gpio_is_valid(ak4375_data->gpio_reset)) {
                ret= gpio_request(ak4375_data->gpio_reset, "akm4375_reset");
                if (ret) {
                        dev_err(&i2c->dev, "akm4375 reset gpio request failed");
                  return ret;
                }
				
			gpio_direction_output(ak4375_data->gpio_reset, 0);
			msleep(20);
			gpio_direction_output(ak4375_data->gpio_reset, 1);
			msleep(20);	
			
        }
	else{
                dev_err(&i2c->dev, "akm4375 reset gpio dt is  invalid");
	}
   ...
   #ifdef ZTE_HIFI_STATE_PROC
	refresh = proc_create("driver/hifi_chip_state", 0644, NULL, &hifi_chip_operations);	
	if (!refresh) {
		printk("weiguohua creat file node driver/hifi_chip_state failed $$$\n");
	}
    #endif	

	akdbgprt("\t[AK4375] probe leave %s(%d)\n",__FUNCTION__,__LINE__);	
	return ret;
}

#if 0
static int __devexit ak4375_i2c_remove(struct i2c_client *client)
{
...
#endif
static struct of_device_id akm_4375_match_table[] = {
...
static struct i2c_driver ak4375_i2c_driver = {
	.driver = {
		...
		.owner = THIS_MODULE,
		//weiguohua add it temp
          .of_match_table = akm_4375_match_table,	
          //weiguohua add it temp
	},
	.probe = ak4375_i2c_probe,
	#if 0
	.remove = __devexit_p(ak4375_i2c_remove),
	#else
