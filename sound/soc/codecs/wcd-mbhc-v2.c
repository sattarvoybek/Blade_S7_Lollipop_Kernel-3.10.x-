...
#ifdef ZTE_MAKE_HEADSET_DETECT_STABLE
#define MBHC_BUTTON_PRESS_THRESHOLD_MIN 2000
#else
...
int hook_press_state = 0;  //wangxiumei add for headset hook test for PV version 

static int hs_state_show(struct seq_file *m, void *v)
{
	return	seq_printf(m, "headset_plug_state: %d \nhook_press_state: %d \n",headset_plug_state,hook_press_state);
}
...
pr_debug("%s: zte debug goto here : %d\n", __func__,
	...
	if (mbhc->headset_jack.jack == NULL) {
		...
		#ifdef CONFIG_ZTE_SUPPORT_HEADSET_VOL_UP_DOWN
		ret = snd_jack_set_key(mbhc->button_jack.jack,
				       SND_JACK_BTN_1,
				       KEY_VOLUMEUP);
		pr_info("%s: snd_jack_set_key SND_JACK_BTN_1  %d \n", __func__,ret);
		if (ret) {
		...
		ret = snd_jack_set_key(mbhc->button_jack.jack,
				       SND_JACK_BTN_2,
				       KEY_VOLUMEDOWN);
		pr_info("%s: snd_jack_set_key SND_JACK_BTN_2 ,ret %d \n", __func__,ret);
		if (ret) {
