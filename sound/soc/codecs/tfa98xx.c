...
//#define DEBUG  //weiguohua delete it for debug 20150619
...
static void tfa98xx_monitor(struct work_struct *work)
{
    ...
    if (TFA98XX_STATUSREG_SPKS & val)
        pr_debug("TFA: SYS_STATUS SPKS detected");
    
    if (!(TFA98XX_STATUSREG_SWS & val))
        pr_debug("TFA: SYS_STATUS SWS stop switching");
...
static void tfa98xx_dsp_init(struct work_struct *work)
{
	...
	pr_debug("tfa profile %d, vstep %d\n", tfa98xx->profile_ctl,
		 tfa98xx->vstep_ctl);

	mutex_lock(&tfa98xx->dsp_init_lock);

	val = snd_soc_read(tfa98xx->codec, TFA98XX_STATUSREG);
	pr_debug("tfa SYS_STATUS: 0x%04x\n", val);
	if (TFA98XX_STATUSREG_ACS & val)
    {
	    ...
	    if (!tfa98xx_dsp_start(tfa98xx, tfa98xx->profile_ctl,
		                 tfa98xx->vstep_ctl))
	    {
		    /* SL: Added stop and restart for cold start up */
...
static int tfa98xx_set_dai_fmt(struct snd_soc_dai *codec_dai, unsigned int fmt)
{
	...
	pr_debug("%s fmt is %d\n",__FUNCTION__,fmt); //weiguohua add it temp
	...
	pr_debug("%s val is %d\n",__FUNCTION__,val); //weiguohua add it temp
...
static const struct soc_enum tfa98xx_enum[] = 
{
...
struct snd_ctl_elem_value  *ucontrol)
{
	...
	for ( i = regs ; i <= rege ; i++ ){
		...
		printk("**tfa98xx  Addr,Reg=(%x, 0x%4x)\n", i, value);
	}

	value = snd_soc_read(codec, 0x80);
		printk("**tfa98xx  Addr,Reg=(0x80, 0x%4x)\n", value);	
    }
	return 0;
}
#endif
...
static struct snd_kcontrol_new tfa98xx_controls[] = {
	...
	},
	{
		...
		//weiguohua 
	SOC_ENUM_EXT("TFA Reg Read", tfa98xx_enum[0], get_test_reg, set_test_reg),
		//weiguohua
};
...
static int tfa98xx_probe(struct snd_soc_codec *codec)
{
   ...
   // struct snd_soc_dapm_context *dapm = &codec->dapm; 
	...
	snd_soc_dapm_ignore_suspend(&codec->dapm, "NXP Output Mixer");
	...
	if(ret)
		return ret;
dev_info(codec->dev, "zte debug in probe %d\n",0); //weiguohua
...
dev_info(codec->dev, "zte debug in probe  success in tfa98xxx_probe%d\n",0); //weiguohua
	return 0;
}
...
//weiguohua add it temp
//weiguohua add it temp
static int tfa9897_parse_dt(struct device *dev,  struct tfa98xx *pdata)
{
        struct device_node *np = dev->of_node;
        u32 gpio_rst;
	gpio_rst = of_get_named_gpio(np, "tfa9897,gpio-rstn", 0);
	if (gpio_rst < 0) {
		dev_err(dev, "Unable to read gpio_rstn\n");
	} else {
		pdata->rst_gpio = gpio_rst;
	}
        return 0;
}
...
//weiguohua add it temp
//	codec = tfa98xx->codec;
//wieguohua add it temp
	i2c_set_clientdata(i2c, tfa98xx);
	
//	codec->dev = &i2c->dev;
//dev_set_name(codec->dev, "%s", "tfa98xx");
	//weiguohua add it temp
        if (i2c->dev.of_node) {
                printk("tfa9897 use device tree\n");

                ret= tfa9897_parse_dt(&i2c->dev, tfa98xx);
                if (ret) {
                        dev_err(&i2c->dev,
                ...
                printk("tfa9897 not use device tree\n");                
        }	
        if (gpio_is_valid(tfa98xx->rst_gpio)) {
                ret= gpio_request(tfa98xx->rst_gpio, "tfa9897_reset");
                if (ret) {
                        dev_err(&i2c->dev, "tfa9897 reset gpio request failed");
                  return ret;
                }
			...
			msleep(20);	
                ...
                dev_err(&i2c->dev, "tfa9897 reset gpio dt is  invalid");
	}
